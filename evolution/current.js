...
function render(){
 ...
 $('#stop-explanation').textContent=`${study.display_label}: ${f(study.initial[maxKey])} → ${f(study.current[maxKey])} nm maximum EPE. Retained PVB changes by ${f(100*(study.current[pvbKey]/study.initial[pvbKey]-1),2)}%, within the fixed 2% cap. The remaining candidate allowance is unused.`;
 $('#previous').disabled=index===0;$('#next').disabled=index===study.points.length-1;
 $('#case-selection').textContent=study.selection;
 $('#stage-explanation').textContent=study.view_id==='metal1'?'Metal27 reaches its best retained Max EPE at candidate 2. Candidates 6 and 7 test local feedback and both worsen the maximum. Two later proposals repeat already measured recipes, triggering the frozen stopping rule.':'This original trajectory stays global and stops after two consecutive proposals recreate already measured recipes. The extra budget alone does not produce sustained local refinement.';
 renderGeometry();
}
fetch('../assets/current/results.json?v=metal27').then(r=>{if(!r.ok)throw Error();return r.json();}).then(d=>{data=d;study=d.trajectories.find(x=>x.view_id===new URLSearchParams(location.search).get('case'))||d.trajectories[0];index=study.points.length-1;$('#trial').disabled=false;$('#trial').max=String(index);document.querySelectorAll('[data-case]').forEach(b=>{const active=b.dataset.case===study.view_id;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));});render();
